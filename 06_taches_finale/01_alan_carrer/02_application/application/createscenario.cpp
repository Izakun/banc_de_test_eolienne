...
createScenario::createScenario(QSqlDatabase *db, QWidget *parent) : QWidget(parent), ui(new Ui::createScenario)
{
    ui->setupUi(this);
    this->setWindowTitle("Banc de test eolienne | Creer scenario");

    this->db = db;
    Actu();

    //connect(ui->combo_scenario,SIGNAL(currentIndexChanged(const QString&)),this,SLOT(Actu()));
}
...
{
    if(ui->combo_scenario->currentData() != NULL && ui->txt_phase->text() != NULL && ui->nb_power->value() != 0 && ui->nb_time->value() != 0){
        phase = new Cphase(ui->combo_scenario->currentData().toInt() ,ui->nb_time->value(),ui->nb_power->value());
        if(phase->insertDB(*db)){
            ui->txt_scenario->clear();
            ui->tab_db->clear();
            qDebug() << ui->combo_scenario->currentData();
            qDebug() << ui->txt_phase->text();
            qDebug() << ui->nb_power->value();
            qDebug() << ui->nb_time->value();
...
void createScenario::Actu()
{
    ...
    while(query.next()){
        ...
        ui->tab_db->setItem(ui->tab_db->rowCount()-1,0, new QTableWidgetItem(query.value("temps").toString()));
