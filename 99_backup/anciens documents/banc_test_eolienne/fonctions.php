...
function hist_releve(){
    ...
    $resultats= $bdd->query("SELECT DATE_FORMAT(date_time, '%d/%m/%Y à %Hh%imin'),eoliennes.nom_eol, scenarios.nom_scenar, releves.puissance, releves.consigne,  releves.force_vent as km, ROUND(releves.force_vent/3.6, 1) as ms
                            FROM releves
                            JOIN `tests` 
...
function nom_scen_use($id_scenario){
        require 'connexion.php';
...
function hist_releve_scenario($id_scenario){
    ...
    $resultats= $bdd->query("SELECT DATE_FORMAT(date_time, '%d/%m/%Y à %Hh%imin'),eoliennes.nom_eol, releves.puissance, releves.consigne,  releves.force_vent as km, ROUND(releves.force_vent/3.6, 1) as ms, tests.reference
                            FROM releves
                            JOIN `tests` 
                            ON releves.ID_releves = tests.ID_releves
                            JOIN `eoliennes`
...
echo $modulo;
	...
	if (is_numeric($modulo))
    	{
	        if ((int) $modulo == $modulo) // <-- HERE
        	{ 	
			$int_pagination = (int)$modulo;
			return $int_pagination;
		}
        	else 
        	{	 
			$int_pagination = (int)$modulo +1;		
        		return $int_pagination;
		}
    	}
	/*if(filter_var($modulo, FILTER_VALIDATE_INT)){
echo 1;
		//var_dump(filter_var($modulo, FILTER_VALIDATE_INT));
...
function btnlistescenar(){
   ...
   echo '<a href="historique.php"  class="btn btn-perso-prim-clair"> historique global <span class="glyphicon glyphicon-chevron-right"></span></a> ';
   
   foreach ($resultatslistscenar as $row){
       if ($row['ID_scenarios'] != 0){
            $nom_scenario = $row['nom_scenar'];
            $id_scenario = $row['ID_scenarios'];
       
            
            echo '<a href="historique.php?scen='.$id_scenario.'"  class="btn btn-perso-prim-clair"> '.$nom_scenario.' <span class="glyphicon glyphicon-chevron-right"></span></a> ';
       }
