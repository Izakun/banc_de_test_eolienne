...
function hist_releve_scenario($id_scenario){
    ...
    $resultats= $bdd->query("SELECT DATE_FORMAT(date_time, '%d/%m/%Y à %Hh%imin'),eoliennes.nom_eol, releves.puissance, releves.consigne,  releves.force_vent as km, ROUND(releves.force_vent/3.6, 1) as ms
                            FROM releves
                            JOIN `tests` 
                            ON releves.ID_releves = tests.ID_releves
                            JOIN `eoliennes`
